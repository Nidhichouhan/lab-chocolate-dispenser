...
function addChocolates(chocolates, color, counter) {
    if (counter > 0) {
...
function dispenseChocolatesOfColor(chocolates, number, color) {
    if (number > chocolates.length)
        ...
        var favoriteChocolates = [];
        var count = 0;
        for (var i = chocolates.length; i >= 0; i--) {
            if (chocolates[i] == color)
                favoriteChocolates[count++] = chocolates.pop();
        }
        return favoriteChocolates;
    } else
        return "Number cannot be zero/negative";
...
//Challenge 2: Dispense 1 rainbow colored colored chocolate for every 3 chocolates of the same color 